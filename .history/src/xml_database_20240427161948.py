...
def generate_xml(password, database):
    ...
    for paciente in pacientes:
        ...
        for muestra in muestras:
            ...
            cursor.execute(f"SELECT m.Microorganism_ID, m.FASTA, m.Kingdom, m.Species, m.Seq_length, sm.qPCR
FROM microorganism m, sample_microorganism sm
WHERE sm.Microorganism_ID=m.Microorganism_ID  AND sm.Sample_ID = {muestra['ID']}")
