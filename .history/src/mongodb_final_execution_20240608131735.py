...
class MongoDBFinalExecution:
    ...
    def run(self, output_csv: str) -> None:
        ...
        for query_name, query_func, *query_args in tqdm(queries):
...
def main() -> int:
    ...
    execution.run(output_csv='./query_optimization/mongodb/query_execution_times.csv')
    
    df = pd.read_csv('./query_optimization/mongodb/query_execution_times.csv')
    # Generación del gráfico de barras
    plt.figure(figsize=(10, 6))
    plt.bar(df["Query"], df["Time_ms"]/1000, color='skyblue')
    plt.title('Execution Time of Each Query')
    plt.xlabel('Query')
    plt.ylabel('Time (s)')
    plt.grid(True)
    plt.savefig("query_optimization/mongodb/figures/query_times_mongodb.png")
    plt.show()
