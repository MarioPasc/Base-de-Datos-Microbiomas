...
def create_venn_diagram(csv_path, output_dir, output_filename):
    ...
    # Seleccionar las queries con más índices únicos (hasta 3 para un diagrama de Venn de 3 conjuntos)
    selected_queries = sorted(index_sets, key=lambda k: len(index_sets[k]), reverse=True)[:3]

    # Crear los conjuntos para el diagrama de Venn
    sets = [index_sets[query] for query in selected_queries]

    # Crear el diagrama de Venn
    plt.figure(figsize=(10, 10))
    
    if len(sets) == 2:
        venn_diagram = venn2(sets, set_labels=selected_queries)
    elif len(sets) == 3:
        venn_diagram = venn3(sets, set_labels=selected_queries)

    # Añadir etiquetas personalizadas con los índices en las intersecciones
    for subset in venn_diagram.subset_labels:
        if subset:
            indices = set.intersection(*(sets[i] for i in range(len(subset)) if subset[i] == '1'))
            venn_diagram.get_label_by_id(subset).set_text('\n'.join(sorted(indices)))
