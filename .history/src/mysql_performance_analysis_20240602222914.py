...
def decode_indices(csv_file_path, output_file_path):
    ...
    decoding_dict = {
        f'I{i+1}': indices_list[i] for i in range(len(indices_list))
    }
...
if __name__ == "__main__":
    ...
    # Find the minimum execution time for each query
    min_times = df_exploded.loc[df_exploded.groupby('query')['times'].idxmin()]

    # Select relevant columns
    min_times_selected = min_times[['times', 'query', 'indices', 'engine']]

    # Rename columns for clarity
    min_times_selected.columns = ['Minimum Time (s)', 'Query', 'Index Combination', 'Engine']

    # Display the dataframe
    min_times_selected.to_csv("./query_optimization/mysql/best_combinations.csv")
