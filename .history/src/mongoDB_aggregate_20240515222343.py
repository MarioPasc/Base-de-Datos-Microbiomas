...
class MongoDBAggregations:
    ...
    def get_samples_and_qpcr_given_microorganism(self, collection_patient:str):
        collection_patient_object = self.db.get_collection(collection_patient)
        pipeline = [
                        {
                            '$unwind': {
                            ...
                            '$match': {
                                'samples.microorganisms.microorganism_id': 'MIC-64254-KRV'
                            }
                        }, {
                            '$project': {
        ...
        results = collection_patient_object.aggregate(pipeline=pipeline)
